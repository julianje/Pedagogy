...
setwd("~/Documents/Projects/Models/Pedagogy/Pedagogy/FinalData/")
data <- read.csv("Model_Uniform.csv") %>% tbl_df %>%
  mutate(Model=paste(LearnC,LearnR,DiscoverC,DiscoverR))
data$Condition<-as.character(data$Condition)
data$ExploreProb<-factor(data$ExploreProb)
levels(data$ExploreProb)=c(".25 Exploration",".5 Exploration",".75 Exploration",".99 Exploration")
levels(data$Decision)=c("Either","Red toy","Yellow toy")

Summarydata <- data %>%
  group_by(ExploreProb,Condition,Model) %>%
  do(data.frame(Decision=prop.table(table(.$Decision)))) %>%
  mutate(Percentage=Decision.Freq*100) %>% select(-Decision.Freq) %>%
  dplyr::rename(Decision=Decision.Var1)
# Get total number of samples (for normalizing later) by subselecting one of the ests.
...
Simpledata <- full_join(ConcreteChoices,NoPrefChoices,by=c("ExploreProb","Condition","Model")) %>% mutate(Perc=Percentage+PercAdd) %>% select(-Percentage,-PercAdd)
rm(ConcreteChoices,NoPrefChoices,data)
```
...
Probs<-plyr::ddply(Probs,c("Condition","Model","Prob","RedToy","SampleSize","NoiseProb"),function(x){dbinom(x$RedToy,x$SampleSize,x$NoiseProb)}) %>% dplyr::rename(Likelihood=V1) %>% tbl_df
...
Summarydata %>%
  filter(Model=="0 0 1 1") %>%
  ggplot(aes(x=Condition,y=Percentage,fill=Decision))+geom_bar(color="black",stat="identity")+facet_grid(~ExploreProb)+
